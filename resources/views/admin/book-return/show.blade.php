...
<div class="row">
      ...
      <form action="" method="POST" class="book_borrower_form">
        ...
        <table class="table">
          ...
          <tr>
            ...
            @else
            <td class="text-success text-wrap">Dikembalikan</td>
            @endif
          </tr>
        </table>
    </div>
  ...
  <div class="col-lg-5">
    ...
    <div class="py-4">
      <a href="{{ route('admin.book-return.index') }}" class="btn btn-primary">Kembali</a>
      @if($book_user->status === 1)
        <button type="submit" class="btn btn-success" data-id="{{ $book_user->id }}" id="book-approved-button">Kembalikan</button>
      @endif
    </div>
...
@push('modal')
@include('admin.book-return.modal.user.show');
@include('admin.book-return.modal.book.show')
@endpush
...
<script>
  $(document).ready(function() {
    ...
    $("#book-approved-button").click(function(e) {
      ...
      Swal.fire({
        title: 'Setuju?',
        text: "Data pengembalian akan disetujui.",
        icon: 'warning',
        ...
        if (result.value) {
          ...
          $.ajax({
            ...
            success: function(data) {
              Swal.fire({
                title: "Berhasil",
                text: "Berhasil menyetujui pengembalian.",
                icon: "success",
